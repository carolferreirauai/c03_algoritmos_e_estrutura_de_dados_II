...
USE SONDAGEM QUADRÁTICA PARA ESTE PROBLEMA.

ENTRADA: A ENTRADA CONSISTE DE VÁRIAS LINHAS:
- A PRIMEIRA LINHA IRÁ CONTER O TAMANHO M DA TABELA (UM NÚMERO INTEIRO), E AS CONSTANTES C1 E C2 PARA A SONDAGEM QUADRÁTICA
...
//sondagem quadratica - hash auxiliar
...
int hash_insert(dado t[], int m, int k, int c1, int c2)
{
    ...
    do 
    {
        ...
        // Se a gaveta estiver vazia (0) ou tiver um dado removido (2)
        if (t[j].status != 1)
        {
            t[j].k = k; 
            t[j].status = 1; 
...
int main()
{
    ...
    //entrada de dados
    //ler tamanho da tabela (m) e as duas constantes (c1 e c2)
    cin >> m >> c1 >> c2;
    ...
    while (cin >> opcao && opcao != 5)
    {
        switch (opcao)
        {
            case 1: //inserir
                cin >> k;
                hash_insert(tabela, m, k, c1, c2);
                break;
                
            case 2: //pesquisar
                cin >> k;
                cout << hash_search(tabela, m, k, c1, c2) << endl;
                break;
                
            case 3: //remover
                cin >> k;
                hash_remove(tabela, m, k, c1, c2);
                break;
